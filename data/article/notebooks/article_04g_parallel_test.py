# %% [markdown]
"""
# Test: Parallel Interior + Sequential Boundaries in same function
"""

# %%
import time
import numpy as np
import xarray as xr
from numba import njit, prange

from seapopym.standard.coordinates import Coordinates, GridPosition
from seapopym.transport import (
    compute_spherical_cell_areas,
    compute_spherical_dx,
    compute_spherical_dy,
    compute_spherical_face_areas_ew,
    compute_spherical_face_areas_ns,
)
from seapopym.transport.numba_kernels import transport_tendency_numba

print("✅ Imports loaded")

# %%
# Setup test data
NY, NX = 121, 191
N_COHORTS = 12
N_ITERATIONS = 20

lat = xr.DataArray(np.linspace(-60, 60, NY), dims=["y"])
lon = xr.DataArray(np.linspace(110, 290, NX), dims=["x"])

cell_areas = compute_spherical_cell_areas(lat, lon).values
face_areas_ew = compute_spherical_face_areas_ew(lat, lon)
face_areas_ns = compute_spherical_face_areas_ns(lat, lon)
dx = compute_spherical_dx(lat, lon).values
dy = compute_spherical_dy(lat, lon).values

x_face_dim = GridPosition.get_face_dim(Coordinates.X, GridPosition.LEFT)
y_face_dim = GridPosition.get_face_dim(Coordinates.Y, GridPosition.LEFT)
ew_area = face_areas_ew.isel({x_face_dim: slice(1, None)}).values
ns_area = face_areas_ns.isel({y_face_dim: slice(1, None)}).values

state = np.random.rand(N_COHORTS, NY, NX).astype(np.float64)
u = np.random.uniform(-0.5, 0.5, (NY, NX)).astype(np.float64)
v = np.random.uniform(-0.5, 0.5, (NY, NX)).astype(np.float64)
D_2d = np.full((NY, NX), 500.0, dtype=np.float64)
mask = np.ones((NY, NX), dtype=np.float64)
mask[40:50, 80:100] = 0
bc = np.array([0, 0, 0, 0], dtype=np.int32)

print(f"✅ Test data: {NY}x{NX}, {N_COHORTS} cohorts")

# %% [markdown]
"""
## Reference: Current guvectorize
"""

# %%
adv_ref = np.zeros((NY, NX), dtype=np.float64)
diff_ref = np.zeros((NY, NX), dtype=np.float64)
transport_tendency_numba(
    state[0], u, v, D_2d, dx, dy, cell_areas, ew_area, ns_area, mask, bc, adv_ref, diff_ref
)


def benchmark_current(n_iter):
    times = []
    for _ in range(n_iter):
        t0 = time.perf_counter()
        for c in range(N_COHORTS):
            transport_tendency_numba(
                state[c],
                u,
                v,
                D_2d,
                dx,
                dy,
                cell_areas,
                ew_area,
                ns_area,
                mask,
                bc,
                adv_ref,
                diff_ref,
            )
        times.append(time.perf_counter() - t0)
    return np.mean(times) * 1000, np.std(times) * 1000


mean_ref, std_ref = benchmark_current(N_ITERATIONS)
print(f"📊 Reference (guvectorize): {mean_ref:.3f} ± {std_ref:.3f} ms")

# Store reference
adv_reference = np.zeros((N_COHORTS, NY, NX), dtype=np.float64)
diff_reference = np.zeros((N_COHORTS, NY, NX), dtype=np.float64)
for c in range(N_COHORTS):
    transport_tendency_numba(
        state[c],
        u,
        v,
        D_2d,
        dx,
        dy,
        cell_areas,
        ew_area,
        ns_area,
        mask,
        bc,
        adv_reference[c],
        diff_reference[c],
    )

# %% [markdown]
"""
## Parallel Interior + Sequential Boundaries
"""


# %%
@njit(parallel=True, fastmath=True, cache=True)
def transport_parallel_hybrid(
    state, u, v, D, dx, dy, cell_areas, ew_area, ns_area, mask, advection_out, diffusion_out
):
    """Parallel interior + sequential boundaries in same function."""
    ny, nx = state.shape

    # =========================================================================
    # PASS 1: INTERIOR CELLS (parallel - no BC checks needed)
    # =========================================================================
    for j in prange(1, ny - 1):
        for i in range(1, nx - 1):
            advection_out[j, i] = 0.0
            diffusion_out[j, i] = 0.0

            if mask[j, i] == 0:
                continue

            c_center = state[j, i]
            u_center = u[j, i]
            v_center = v[j, i]
            d_center = D[j, i]
            dx_center = dx[j, i]
            dy_center = dy[j, i]
            area = cell_areas[j, i]

            adv_div = 0.0
            diff_div = 0.0

            # East (i+1) - guaranteed valid for interior
            if mask[j, i + 1] != 0:
                c_east = state[j, i + 1]
                u_east = u[j, i + 1]
                d_east = D[j, i + 1]
                dx_east = dx[j, i + 1]
                face_area = ew_area[j, i]
                u_face = 0.5 * (u_center + u_east)
                if u_face > 0:
                    c_up = c_center
                else:
                    c_up = c_east
                adv_div = adv_div + u_face * c_up * face_area
                d_face = 0.5 * (d_center + d_east)
                dx_face = 0.5 * (dx_center + dx_east)
                diff_div = diff_div - d_face * (c_east - c_center) / dx_face * face_area

            # West (i-1)
            if mask[j, i - 1] != 0:
                c_west = state[j, i - 1]
                u_west = u[j, i - 1]
                d_west = D[j, i - 1]
                dx_west = dx[j, i - 1]
                face_area = ew_area[j, i - 1]
                u_face = 0.5 * (u_west + u_center)
                if u_face > 0:
                    c_up = c_west
                else:
                    c_up = c_center
                adv_div = adv_div - u_face * c_up * face_area
                d_face = 0.5 * (d_west + d_center)
                dx_face = 0.5 * (dx_west + dx_center)
                diff_div = diff_div + d_face * (c_center - c_west) / dx_face * face_area

            # North (j+1)
            if mask[j + 1, i] != 0:
                c_north = state[j + 1, i]
                v_north = v[j + 1, i]
                d_north = D[j + 1, i]
                dy_north = dy[j + 1, i]
                face_area = ns_area[j, i]
                v_face = 0.5 * (v_center + v_north)
                if v_face > 0:
                    c_up = c_center
                else:
                    c_up = c_north
                adv_div = adv_div + v_face * c_up * face_area
                d_face = 0.5 * (d_center + d_north)
                dy_face = 0.5 * (dy_center + dy_north)
                diff_div = diff_div - d_face * (c_north - c_center) / dy_face * face_area

            # South (j-1)
            if mask[j - 1, i] != 0:
                c_south = state[j - 1, i]
                v_south = v[j - 1, i]
                d_south = D[j - 1, i]
                dy_south = dy[j - 1, i]
                face_area = ns_area[j - 1, i]
                v_face = 0.5 * (v_south + v_center)
                if v_face > 0:
                    c_up = c_south
                else:
                    c_up = c_center
                adv_div = adv_div - v_face * c_up * face_area
                d_face = 0.5 * (d_south + d_center)
                dy_face = 0.5 * (dy_south + dy_center)
                diff_div = diff_div + d_face * (c_center - c_south) / dy_face * face_area

            advection_out[j, i] = -adv_div / area
            diffusion_out[j, i] = -diff_div / area

    # =========================================================================
    # PASS 2: BOUNDARY CELLS (sequential - with BC checks)
    # For CLOSED boundaries (bc=0), boundary cells have no flux across domain edges
    # =========================================================================

    # Top row (j = 0)
    for i in range(nx):
        j = 0
        advection_out[j, i] = 0.0
        diffusion_out[j, i] = 0.0

        if mask[j, i] == 0:
            continue

        c_center = state[j, i]
        u_center = u[j, i]
        v_center = v[j, i]
        d_center = D[j, i]
        dx_center = dx[j, i]
        dy_center = dy[j, i]
        area = cell_areas[j, i]

        adv_div = 0.0
        diff_div = 0.0

        # East
        if i + 1 < nx and mask[j, i + 1] != 0:
            c_east = state[j, i + 1]
            u_east = u[j, i + 1]
            d_east = D[j, i + 1]
            dx_east = dx[j, i + 1]
            face_area = ew_area[j, i]
            u_face = 0.5 * (u_center + u_east)
            if u_face > 0:
                c_up = c_center
            else:
                c_up = c_east
            adv_div = adv_div + u_face * c_up * face_area
            d_face = 0.5 * (d_center + d_east)
            dx_face = 0.5 * (dx_center + dx_east)
            diff_div = diff_div - d_face * (c_east - c_center) / dx_face * face_area

        # West
        if i - 1 >= 0 and mask[j, i - 1] != 0:
            c_west = state[j, i - 1]
            u_west = u[j, i - 1]
            d_west = D[j, i - 1]
            dx_west = dx[j, i - 1]
            face_area = ew_area[j, i - 1]
            u_face = 0.5 * (u_west + u_center)
            if u_face > 0:
                c_up = c_west
            else:
                c_up = c_center
            adv_div = adv_div - u_face * c_up * face_area
            d_face = 0.5 * (d_west + d_center)
            dx_face = 0.5 * (dx_west + dx_center)
            diff_div = diff_div + d_face * (c_center - c_west) / dx_face * face_area

        # North (j+1 exists)
        if mask[j + 1, i] != 0:
            c_north = state[j + 1, i]
            v_north = v[j + 1, i]
            d_north = D[j + 1, i]
            dy_north = dy[j + 1, i]
            face_area = ns_area[j, i]
            v_face = 0.5 * (v_center + v_north)
            if v_face > 0:
                c_up = c_center
            else:
                c_up = c_north
            adv_div = adv_div + v_face * c_up * face_area
            d_face = 0.5 * (d_center + d_north)
            dy_face = 0.5 * (dy_center + dy_north)
            diff_div = diff_div - d_face * (c_north - c_center) / dy_face * face_area

        # South: CLOSED boundary, no flux

        advection_out[j, i] = -adv_div / area
        diffusion_out[j, i] = -diff_div / area

    # Bottom row (j = ny - 1)
    for i in range(nx):
        j = ny - 1
        advection_out[j, i] = 0.0
        diffusion_out[j, i] = 0.0

        if mask[j, i] == 0:
            continue

        c_center = state[j, i]
        u_center = u[j, i]
        v_center = v[j, i]
        d_center = D[j, i]
        dx_center = dx[j, i]
        dy_center = dy[j, i]
        area = cell_areas[j, i]

        adv_div = 0.0
        diff_div = 0.0

        # East
        if i + 1 < nx and mask[j, i + 1] != 0:
            c_east = state[j, i + 1]
            u_east = u[j, i + 1]
            d_east = D[j, i + 1]
            dx_east = dx[j, i + 1]
            face_area = ew_area[j, i]
            u_face = 0.5 * (u_center + u_east)
            if u_face > 0:
                c_up = c_center
            else:
                c_up = c_east
            adv_div = adv_div + u_face * c_up * face_area
            d_face = 0.5 * (d_center + d_east)
            dx_face = 0.5 * (dx_center + dx_east)
            diff_div = diff_div - d_face * (c_east - c_center) / dx_face * face_area

        # West
        if i - 1 >= 0 and mask[j, i - 1] != 0:
            c_west = state[j, i - 1]
            u_west = u[j, i - 1]
            d_west = D[j, i - 1]
            dx_west = dx[j, i - 1]
            face_area = ew_area[j, i - 1]
            u_face = 0.5 * (u_west + u_center)
            if u_face > 0:
                c_up = c_west
            else:
                c_up = c_center
            adv_div = adv_div - u_face * c_up * face_area
            d_face = 0.5 * (d_west + d_center)
            dx_face = 0.5 * (dx_west + dx_center)
            diff_div = diff_div + d_face * (c_center - c_west) / dx_face * face_area

        # North: CLOSED boundary, no flux

        # South (j-1 exists)
        if mask[j - 1, i] != 0:
            c_south = state[j - 1, i]
            v_south = v[j - 1, i]
            d_south = D[j - 1, i]
            dy_south = dy[j - 1, i]
            face_area = ns_area[j - 1, i]
            v_face = 0.5 * (v_south + v_center)
            if v_face > 0:
                c_up = c_south
            else:
                c_up = c_center
            adv_div = adv_div - v_face * c_up * face_area
            d_face = 0.5 * (d_south + d_center)
            dy_face = 0.5 * (dy_south + dy_center)
            diff_div = diff_div + d_face * (c_center - c_south) / dy_face * face_area

        advection_out[j, i] = -adv_div / area
        diffusion_out[j, i] = -diff_div / area

    # Left column (i = 0), excluding corners
    for j in range(1, ny - 1):
        i = 0
        advection_out[j, i] = 0.0
        diffusion_out[j, i] = 0.0

        if mask[j, i] == 0:
            continue

        c_center = state[j, i]
        u_center = u[j, i]
        v_center = v[j, i]
        d_center = D[j, i]
        dx_center = dx[j, i]
        dy_center = dy[j, i]
        area = cell_areas[j, i]

        adv_div = 0.0
        diff_div = 0.0

        # East (i+1 exists)
        if mask[j, i + 1] != 0:
            c_east = state[j, i + 1]
            u_east = u[j, i + 1]
            d_east = D[j, i + 1]
            dx_east = dx[j, i + 1]
            face_area = ew_area[j, i]
            u_face = 0.5 * (u_center + u_east)
            if u_face > 0:
                c_up = c_center
            else:
                c_up = c_east
            adv_div = adv_div + u_face * c_up * face_area
            d_face = 0.5 * (d_center + d_east)
            dx_face = 0.5 * (dx_center + dx_east)
            diff_div = diff_div - d_face * (c_east - c_center) / dx_face * face_area

        # West: CLOSED boundary, no flux

        # North
        if mask[j + 1, i] != 0:
            c_north = state[j + 1, i]
            v_north = v[j + 1, i]
            d_north = D[j + 1, i]
            dy_north = dy[j + 1, i]
            face_area = ns_area[j, i]
            v_face = 0.5 * (v_center + v_north)
            if v_face > 0:
                c_up = c_center
            else:
                c_up = c_north
            adv_div = adv_div + v_face * c_up * face_area
            d_face = 0.5 * (d_center + d_north)
            dy_face = 0.5 * (dy_center + dy_north)
            diff_div = diff_div - d_face * (c_north - c_center) / dy_face * face_area

        # South
        if mask[j - 1, i] != 0:
            c_south = state[j - 1, i]
            v_south = v[j - 1, i]
            d_south = D[j - 1, i]
            dy_south = dy[j - 1, i]
            face_area = ns_area[j - 1, i]
            v_face = 0.5 * (v_south + v_center)
            if v_face > 0:
                c_up = c_south
            else:
                c_up = c_center
            adv_div = adv_div - v_face * c_up * face_area
            d_face = 0.5 * (d_south + d_center)
            dy_face = 0.5 * (dy_south + dy_center)
            diff_div = diff_div + d_face * (c_center - c_south) / dy_face * face_area

        advection_out[j, i] = -adv_div / area
        diffusion_out[j, i] = -diff_div / area

    # Right column (i = nx - 1), excluding corners
    for j in range(1, ny - 1):
        i = nx - 1
        advection_out[j, i] = 0.0
        diffusion_out[j, i] = 0.0

        if mask[j, i] == 0:
            continue

        c_center = state[j, i]
        u_center = u[j, i]
        v_center = v[j, i]
        d_center = D[j, i]
        dx_center = dx[j, i]
        dy_center = dy[j, i]
        area = cell_areas[j, i]

        adv_div = 0.0
        diff_div = 0.0

        # East: CLOSED boundary, no flux

        # West (i-1 exists)
        if mask[j, i - 1] != 0:
            c_west = state[j, i - 1]
            u_west = u[j, i - 1]
            d_west = D[j, i - 1]
            dx_west = dx[j, i - 1]
            face_area = ew_area[j, i - 1]
            u_face = 0.5 * (u_west + u_center)
            if u_face > 0:
                c_up = c_west
            else:
                c_up = c_center
            adv_div = adv_div - u_face * c_up * face_area
            d_face = 0.5 * (d_west + d_center)
            dx_face = 0.5 * (dx_west + dx_center)
            diff_div = diff_div + d_face * (c_center - c_west) / dx_face * face_area

        # North
        if mask[j + 1, i] != 0:
            c_north = state[j + 1, i]
            v_north = v[j + 1, i]
            d_north = D[j + 1, i]
            dy_north = dy[j + 1, i]
            face_area = ns_area[j, i]
            v_face = 0.5 * (v_center + v_north)
            if v_face > 0:
                c_up = c_center
            else:
                c_up = c_north
            adv_div = adv_div + v_face * c_up * face_area
            d_face = 0.5 * (d_center + d_north)
            dy_face = 0.5 * (dy_center + dy_north)
            diff_div = diff_div - d_face * (c_north - c_center) / dy_face * face_area

        # South
        if mask[j - 1, i] != 0:
            c_south = state[j - 1, i]
            v_south = v[j - 1, i]
            d_south = D[j - 1, i]
            dy_south = dy[j - 1, i]
            face_area = ns_area[j - 1, i]
            v_face = 0.5 * (v_south + v_center)
            if v_face > 0:
                c_up = c_south
            else:
                c_up = c_center
            adv_div = adv_div - v_face * c_up * face_area
            d_face = 0.5 * (d_south + d_center)
            dy_face = 0.5 * (dy_south + dy_center)
            diff_div = diff_div + d_face * (c_center - c_south) / dy_face * face_area

        advection_out[j, i] = -adv_div / area
        diffusion_out[j, i] = -diff_div / area


# %%
# Warmup
adv_par = np.zeros((NY, NX), dtype=np.float64)
diff_par = np.zeros((NY, NX), dtype=np.float64)
transport_parallel_hybrid(
    state[0], u, v, D_2d, dx, dy, cell_areas, ew_area, ns_area, mask, adv_par, diff_par
)


def benchmark_parallel(n_iter):
    times = []
    for _ in range(n_iter):
        t0 = time.perf_counter()
        for c in range(N_COHORTS):
            transport_parallel_hybrid(
                state[c], u, v, D_2d, dx, dy, cell_areas, ew_area, ns_area, mask, adv_par, diff_par
            )
        times.append(time.perf_counter() - t0)
    return np.mean(times) * 1000, np.std(times) * 1000


mean_par, std_par = benchmark_parallel(N_ITERATIONS)

# Validate
adv_par_results = np.zeros((N_COHORTS, NY, NX), dtype=np.float64)
diff_par_results = np.zeros((N_COHORTS, NY, NX), dtype=np.float64)
for c in range(N_COHORTS):
    transport_parallel_hybrid(
        state[c],
        u,
        v,
        D_2d,
        dx,
        dy,
        cell_areas,
        ew_area,
        ns_area,
        mask,
        adv_par_results[c],
        diff_par_results[c],
    )

adv_diff = np.abs(adv_reference - adv_par_results).max()
diff_diff = np.abs(diff_reference - diff_par_results).max()

print(f"📊 Parallel hybrid: {mean_par:.3f} ± {std_par:.3f} ms")
print(f"   Speedup vs reference: {mean_ref / mean_par:.2f}x")
print(f"   Max diff: adv={adv_diff:.2e}, diff={diff_diff:.2e}")
print(f"   Valid: {'✅' if adv_diff < 1e-10 and diff_diff < 1e-10 else '❌'}")

# %%
print("\n" + "=" * 60)
print("📊 SUMMARY")
print("=" * 60)
print(f"Reference (guvectorize): {mean_ref:.3f} ms")
print(f"Parallel hybrid:         {mean_par:.3f} ms (speedup: {mean_ref / mean_par:.2f}x)")
