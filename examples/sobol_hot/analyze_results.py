"""Analyze Sobol results for Station HOT and produce summary figure.
...
Usage:
    uv run python examples/sobol_hot/analyze_results.py
"""
...
def plot_mean_indices(results, qoi_names):
    ...
    for i, qoi in enumerate(qoi_names):
        ...
        ax.bar(x - width / 2, s1_mean, width, yerr=s1_conf_mean, capsize=4,
               label="S1 (first-order)", color="#4C72B0", alpha=0.85)
        ax.bar(x + width / 2, st_mean, width, yerr=st_conf_mean, capsize=4,
    ...
    fig.suptitle(
        f"Sobol Sensitivity — HOT (N={N_SAMPLES}, mean over {N_POINTS} points)",
...
def main():
    ...
    print("Analyzing Sobol results for Station HOT...")
