...
Domain: 20x20 degrees around Station HOT (23N, 158W = 202E)
Tropical Pacific waters: SST ~20 to 30C (warmest of the six stations).
...
Usage:
    uv run python examples/sobol_hot/run_sobol.py
"""
...
def build_config() -> Config:
    ...
    forcings = {
        ...
        "u": ds["u"], "v": ds["v"], "D": ds["D"],
        "dx": ds["dx"], "dy": ds["dy"],
        "face_height": ds["face_height"], "face_width": ds["face_width"],
        "cell_area": ds["cell_area"], "mask": ds["mask"],
    }
    ...
    return Config.from_dict(
        {
            ...
            "initial_state": {
                "biomass": xr.DataArray(
                    ...
                    dims=["Y", "X"], coords={"Y": lat, "X": lon},
                ),
                "production": xr.DataArray(
                    np.zeros((NY, NX, N_COHORTS), dtype=np.float32),
                    dims=["Y", "X", "C"], coords={"Y": lat, "X": lon},
                ),
            },
            "execution": {
...
def main():
    logger.info("=" * 70)
    logger.info("SOBOL SENSITIVITY — HOT (real data, 20x20, 30 cohorts)")
    logger.info("=" * 70)
