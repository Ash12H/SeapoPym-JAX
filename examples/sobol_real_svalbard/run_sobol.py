"""Sobol sensitivity analysis on Svalbard real SEAPODYM data.
...
Domain: 20x20 degrees around Svalbard (78N, 15E)
Arctic waters: SST ~-3 to 16C (coldest of the three stations).
...
Usage:
    uv run python examples/sobol_real_svalbard/run_sobol.py
"""
...
# Extraction points around Svalbard (Y, X indices within 20x20 grid)
# Svalbard is at ~78N, 15E -> grid index ~(9, 9) (center of domain)
EXTRACTION_POINTS = [
    (9, 9),    # Svalbard center (~78N, 15E)
    (4, 9),    # South (~73N, 15E) — Norwegian Sea
    (14, 9),   # North (~83N, 15E) — Arctic Ocean
    (9, 4),    # West (~78N, 10E) — Greenland Sea
    (9, 14),   # East (~78N, 20E) — Barents Sea
]
...
def main():
    logger.info("=" * 70)
    logger.info("SOBOL SENSITIVITY — Svalbard (real data, 20x20, 30 cohorts)")
    logger.info("=" * 70)
