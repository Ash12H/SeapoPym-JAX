"""Extract forcings for Svalbard Sobol experiment from real SEAPODYM data.

Reads the aggregated 2D PHY Zarr and extracts a 20x20 degree domain around
Svalbard (78N, 15E), time subset 2000-2004 (60 monthly steps).
...
Usage:
    uv run python examples/sobol_real_svalbard/generate_forcings.py
"""
...
def main():
    print("Extracting Svalbard forcings from real SEAPODYM data...")
    ...
    # Load and subset — no longitude conversion needed (already positive)
    ...
    lon = ds.longitude.values.astype(DTYPE)
    ...
    # pp is in mg/m2/day, model expects g/m2/s
    ...
    out = xr.Dataset(
        ...
        attrs={
            "title": "Svalbard forcings from NP-ERA5-NFIX SEAPODYM 2D PHY",
