"""Sobol sensitivity analysis on Station Papa real SEAPODYM data.
...
Domain: 20x20 degrees around Station Papa (50N, 145W = 215E)
Subarctic waters: SST ~3-14C (much colder than Station HOT).
...
Usage:
    uv run python examples/sobol_real_pacific/run_sobol.py
"""
...
N_COHORTS = 30
N_SAMPLES = 2**14  # Power of 2 — total evals: N × (D+2) = 16384 × 7 = 114688
# Small 20x20 grid -> large batch sizes possible.
BATCH_SIZE = 2**14 * 2
CHUNK_SIZE = 500

# Extraction points around Station Papa (Y, X indices within 20x20 grid)
# Station Papa is at ~50N, 145W (215E) -> grid index ~(9, 9) (center of domain)
EXTRACTION_POINTS = [
    (9, 9),    # Station Papa center (~50N, 215E)
    (4, 9),    # South of Papa (~45N, 215E)
    (14, 9),   # North of Papa (~55N, 215E)
    (9, 4),    # West of Papa (~50N, 210E)
    (9, 14),   # East of Papa (~50N, 220E)
]
...
def main():
    logger.info("=" * 70)
    logger.info("SOBOL SENSITIVITY — Station Papa (real data, 20x20, 30 cohorts)")
    logger.info("=" * 70)
