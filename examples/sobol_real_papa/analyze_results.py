"""Analyze Sobol results for Station Papa and produce summary figure.
...
Usage:
    uv run python examples/sobol_real_pacific/analyze_results.py
"""
...
def load_and_compute():
    ...
    # Identify available QoI (non-NaN)
    qoi_names = []
    ...
    for qoi in qoi_names:
        ...
        results[qoi] = {
            "S1": np.array(s1_list),       # (n_points, n_params)
            "ST": np.array(st_list),
...
def plot_mean_indices(results, qoi_names):
    ...
    fig.suptitle(
        f"Sobol Sensitivity — Station Papa (N={N_SAMPLES}, mean over {N_POINTS} points)",
...
def main():
    ...
    print("Analyzing Sobol results for Station Papa...")
