"""Sobol sensitivity analysis on Station HOT real SEAPODYM data.
...
Domain: 20x20 degrees around Station HOT (22.75N, 158W)
GPU memory: small grid -> large batch sizes possible.
...
Usage:
    uv run python examples/sobol_real_hot/run_sobol.py
"""
...
N_COHORTS = 30
N_SAMPLES = 2**14  # Power of 2 — total evals: N × (D+2) = 128 × 7 = 896
# Small 20×20 grid → carry is tiny (~0.6 MB/element), large batches possible.
BATCH_SIZE = 2**14 * 2
CHUNK_SIZE = 500

# Extraction points around Station HOT (Y, X indices within 20×20 grid)
# Station HOT is at ~22.75°N, 202°E → grid index ~(9, 9) (center of domain)
EXTRACTION_POINTS = [
    (9, 9),  # Station HOT center (~23°N, 202°E)
    (5, 9),  # South of HOT (~18°N, 202°E)
    (14, 9),  # North of HOT (~28°N, 202°E)
    (9, 5),  # West of HOT (~23°N, 198°E)
    (9, 14),  # East of HOT (~23°N, 207°E)
]
...
def main():
    logger.info("=" * 70)
    logger.info("SOBOL SENSITIVITY — Station HOT (real data, 20x20, 30 cohorts)")
    logger.info("=" * 70)
