"""Extract forcings for Station HOT Sobol experiment from real SEAPODYM data.

Reads the aggregated 2D PHY Zarr and extracts a 20x20 degree domain around
Station HOT (22.75N, 158W = 202E), time subset 2000-2004 (60 monthly steps).
...
computed from the coordinate grid. Diffusion D is set to a constant 500 m2/s.
Currents (u, v) are time-averaged from monthly fields.

Usage:
    uv run python examples/sobol_real_hot/generate_forcings.py
"""
...
# =============================================================================
# Domain — 20x20 degrees around Station HOT (22.75N, 158W = 202E)
# =============================================================================
LAT_RANGE = (13.5, 32.5)
LON_RANGE = (-167.5, -148.5)  # 192.5E-211.5E in -180/180 convention
...
def main():
    print("Extracting Station HOT forcings from real SEAPODYM data...")
    ...
    temperature = np.maximum(temperature, 0.0)  # clamp to 0C for biological functions (exp/log)
    # pp is in mg/m2/day, model expects g/m2/s → multiply by 1e-3 / 86400
    ...
    out = xr.Dataset(
        ...
        attrs={
            "title": "Station HOT forcings from NP-ERA5-NFIX SEAPODYM 2D PHY",
