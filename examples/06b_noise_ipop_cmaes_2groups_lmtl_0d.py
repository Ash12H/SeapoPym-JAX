...
# %%
N_PARAMS = 5
INITIAL_POPSIZE = 4 + int(3 * math.log(N_PARAMS))
...
BOUNDS = {
    "lambda_0": (BOUNDS_MULTIPLICATOR[0] * TRUE_PARAMS["lambda_0"], BOUNDS_MULTIPLICATOR[1] * TRUE_PARAMS["lambda_0"]),
    "gamma_lambda": (BOUNDS_MULTIPLICATOR[0] * TRUE_PARAMS["gamma_lambda"], BOUNDS_MULTIPLICATOR[1] * TRUE_PARAMS["gamma_lambda"]),
    "tau_r_0": (BOUNDS_MULTIPLICATOR[0] * TRUE_PARAMS["tau_r_0"], BOUNDS_MULTIPLICATOR[1] * TRUE_PARAMS["tau_r_0"]),
    "gamma_tau_r": (BOUNDS_MULTIPLICATOR[0] * TRUE_PARAMS["gamma_tau_r"], BOUNDS_MULTIPLICATOR[1] * TRUE_PARAMS["gamma_tau_r"]),
...
temp_surface = MEAN_TEMPERATURE + (MEAN_TEMPERATURE / 2) * np.sin(2 * np.pi * doy / 365.0)
temp_deep = (MEAN_TEMPERATURE / 2) + (MEAN_TEMPERATURE / 4) * np.sin(2 * np.pi * doy / 365.0)
temp_4d = np.stack(
    [
...
# %%
print("=" * 60)
print("IPOP-CMA-ES with 2 Functional Groups (Twin Experiment, Noisy)")
