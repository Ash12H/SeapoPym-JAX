...
N_WARMUP = 500  # enough for dense mass matrix adaptation (10 free params for 4×4)
N_SAMPLES = 500
NUTS_SEED = 0
...
if __name__ == "__main__":
    ...
    for chain_idx, mode in enumerate(cmaes_result.modes):
        ...
        print(f"  Fixed: tau_r_0={float(fixed_params['tau_r_0']):.4g}, "
    ...
    for chain_idx, (result, fixed) in enumerate(chain_results):
        print(f"\n--- Chain {chain_idx + 1} ---")
        print(f"  Fixed: tau_r_0={float(fixed['tau_r_0']):.4g}, "
              f"gamma_tau_r={float(fixed['gamma_tau_r']):.4g}")
    ...
    if corner is not None:
        ...
        for ci, (result, _) in enumerate(chain_results):
            samples_array = np.column_stack([
                np.array(result.samples[p].flatten()) for p in all_nuts_params
            ])
        ...
        # Manual legend
        legend_handles = [
            Patch(facecolor=chain_colors[ci], label=f"Mode {ci + 1}")
    ...
    ax_bio.scatter(
        ...
        c="red", s=20, zorder=5, label=f"Observations ({OBS_FRACTION:.0%})",
    )
    ...
    for ci, (result, fixed) in enumerate(chain_results):
        ...
        ax_bio.plot(time_days, cmaes_bio_np, "-", color=color, linewidth=1.5,
