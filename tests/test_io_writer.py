...
def configure_model(bp):
    pass  # Nothing to configure for this dummy test
...
def test_memory_writer_integration(simple_config, simple_blueprint, initial_state):
    ...
    # Note: run loop is: save initial, then while current < end, step and save.
    # Initial: Jan 1 (saved)
    # Jan 1 < Jan 4 -> step -> Jan 2 (saved)
    # Jan 2 < Jan 4 -> step -> Jan 3 (saved)
    # Jan 3 < Jan 4 -> step -> Jan 4 (saved)
    # Loop ends (Jan 4 is not < Jan 4).
    # Total: 4 timesteps.
    assert len(results[Coordinates.T.value]) == 4
...
def test_zarr_writer_integration(simple_config, simple_blueprint, initial_state, tmp_path):
    ...
    controller = SimulationController(simple_config)
    controller.setup(configure_model, initial_state, output_path=output_path)
    controller.run()
    ...
    # Verify data persistence by reloading manually
    ds_loaded = xr.open_zarr(output_path)
    assert ds_loaded.equals(results)
...
def test_variable_selection(simple_config, simple_blueprint, initial_state):
    ...
    controller.setup(
        ...
        initial_state,
        output_path=None,
        output_variables=["biomass"],  # Only biomass
    )
    controller.run()
...
def test_metadata_storage(simple_config, simple_blueprint, initial_state):
    ...
    controller = SimulationController(simple_config)
    controller.setup(configure_model, initial_state, output_path=None, output_metadata=meta)
    controller.run()
