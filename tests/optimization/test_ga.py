...
class TestGAOptimizerRunLossFn:
    ...
    def test_respects_n_generations(self):
        ...
        obj = Objective(observations=jnp.zeros(1), transform=lambda o: o["out"])
        opt = GAOptimizer([(obj, "mse", 1.0)], bounds={"x": (0.0, 10.0)}, popsize=8, seed=42)
    ...
    def test_reproducible_with_seed(self):
        ...
        obj = Objective(observations=jnp.zeros(1), transform=lambda o: o["out"])
        opt1 = GAOptimizer([(obj, "mse", 1.0)], bounds={"x": (0.0, 10.0)}, popsize=8, seed=123)
        opt2 = GAOptimizer([(obj, "mse", 1.0)], bounds={"x": (0.0, 10.0)}, popsize=8, seed=123)
