...
class TestGARun:
    def test_minimizes_quadratic(self):
        ...
        result = opt.run(lambda p: (p["x"] - 3.0) ** 2, max_gen=100)
    ...
    def test_multivariate(self):
        ...
        assert float(result.params["x"]) == pytest.approx(1.0, abs=1.0)
        assert float(result.params["y"]) == pytest.approx(2.0, abs=1.0)

    def test_reproducible_with_seed(self):
        def loss_fn(params):
            return params["x"] ** 2

        opt1 = GAOptimizer(bounds={"x": (0.0, 10.0)}, initial_params={"x": jnp.array(5.0)}, popsize=8, seed=123)
        opt2 = GAOptimizer(bounds={"x": (0.0, 10.0)}, initial_params={"x": jnp.array(5.0)}, popsize=8, seed=123)
    ...
    def test_loss_history_decreasing(self):
        ...
        result = opt.run(lambda p: (p["x"] - 5.0) ** 2, max_gen=50, patience=999)
