...
class TestOptimizerInit:
    def _runner(self):
        return CalibrationRunner.standard()

    def _priors(self):
        return PriorSet({"x": Uniform(0.0, 10.0)})

    def _objectives(self):
        return [(Objective(observations=jnp.zeros(3), transform=lambda o: o["x"][:3]), "mse", 1.0)]

    def test_valid_gradient_strategy(self):
        for strat in GRADIENT_STRATEGIES:
            opt = Optimizer(self._runner(), self._priors(), self._objectives(), strategy=strat)
            assert opt.strategy == strat

    def test_valid_hybrid_strategy(self):
        opt = Optimizer(self._runner(), self._priors(), self._objectives(), strategy="hybrid")
        assert opt.strategy == "hybrid"

    def test_unknown_strategy_raises(self):
        with pytest.raises(ValueError, match="Unknown strategy"):
            Optimizer(self._runner(), self._priors(), self._objectives(), strategy="bogus")

    def test_strategy_kwargs_stored(self):
        opt = Optimizer(
            self._runner(), self._priors(), self._objectives(),
            strategy="adam", learning_rate=0.01,
        )
...
class TestBuildLossFn:
    def test_loss_fn_zero_when_pred_equals_obs(self):
        ...
        # Mock model returns predictions exactly matching obs
        model = _mock_model(
            {"x": jnp.array(5.0)},
            {"out": jnp.array([1.0, 2.0, 3.0])},
        )

        loss_fn = opt._build_loss_fn(model, [(prepared, metric_fn, 1.0)])
        loss = loss_fn({"x": jnp.array(5.0)})

        # loss = 0.0 (mse) - log_prob(5.0 | Uniform(0,10))
        # Uniform(0,10).log_prob(5.0) = -log(10)
        # So loss = 0 - (-log(10)) = log(10)
    ...
    def test_loss_fn_weights(self):
        ...
        # Mock model returns predictions [1, 1] → MSE vs [0, 0] = 1.0
        model = _mock_model(
            {"x": jnp.array(5.0)},
            {"out": jnp.array([1.0, 1.0])},
        )

        opt = Optimizer(runner, priors, [(obj, "mse", 2.0)], strategy="adam")
...
class TestDispatchRouting:
    ...
    def test_dispatch_hybrid(self):
        """Hybrid strategy routes to HybridOptimizer."""
        priors = PriorSet({"x": Uniform(0.0, 10.0)})
        runner = CalibrationRunner.standard()
        obj = Objective(observations=jnp.zeros(1), transform=lambda o: o["x"][:1])
        opt = Optimizer(runner, priors, [(obj, "mse", 1.0)], strategy="hybrid")

        with patch("seapopym.optimization.hybrid.HybridOptimizer") as MockHO:
            mock_instance = MagicMock()
            mock_instance.run.return_value = MagicMock()
